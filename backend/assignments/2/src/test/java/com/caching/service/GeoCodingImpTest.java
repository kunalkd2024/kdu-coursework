...
public class GeoCodingImpTest {
    ...
    @Value("${api-key}") // Replace with the actual key injection method you are using
...
//
    ...
    void testGetGeoCodeNegative() {
        try {
            ...
            // Check for 4xx status codes for client errors
            assertTrue(resultCode >= HttpStatus.BAD_REQUEST.value() &&
                            resultCode < HttpStatus.INTERNAL_SERVER_ERROR.value(),
                    "HTTP Status Code should be 4xx for client error. Actual code: " + resultCode);
        } catch (MethodArgumentTypeMismatchException e) {
            // Handle the exception gracefully and mark the test as passed
            System.out.println("MethodArgumentTypeMismatchException occurred. This is expected in the negative test case.");
        } catch (Exception e) {
